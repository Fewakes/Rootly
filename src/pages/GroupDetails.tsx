...
export default function GroupDetails() {
  ...
  const entity = useMemo(
    () => (id ? ({ id, type: 'group' } as AssignEntity) : null),
    [id],
  );
  ...
  const INITIAL_ITEM_COUNT = 2; // The number of items to show before expanding
  ...
  const handleAddNote = useCallback(async () => {
    ...
    await groupNotesService.create({
  ...
  const handleUpdateNote = useCallback(async () => {
    ...
    await groupNotesService.update(editingNote.id, {
  ...
  const handleDeleteNote = useCallback(
    async (noteId: string) => {
      await groupNotesService.deleteById(noteId);
      await refetchNotes();
  ...
  const handleAddTask = useCallback(async () => {
    ...
    await groupTasksService.create({
      title: newTaskTitle,
  ...
  const handleToggleTask = useCallback(
    async (task: Task) => {
      await groupTasksService.update(task.id, { completed: !task.completed });
      await refetchTasks();
    },
    [refetchTasks],
  );
  ...
  const handleDeleteTask = useCallback(
    async (taskId: string) => {
      await groupTasksService.deleteById(taskId);
      await refetchTasks();
            ...
            <CardContent className="space-y-6">
              <div>
                <div className="flex justify-between items-center text-sm mb-1">
                  <span className="text-muted-foreground">Group Rank</span>
                  <span className="font-semibold">
                    Top{' '}
                    {group?.rank && group?.total_groups
                      ? Math.ceil((group.rank / group.total_groups) * 100)
                      : '-'}
                    % ({group?.rank} of {group?.total_groups})
                  </span>
                </div>
                <Progress
                  value={
                    group?.rank && group?.total_groups
                      ? 100 - (group.rank / group.total_groups) * 100
                      : 0
                  }
